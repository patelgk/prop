    ...
    try {
      if (isLogin) {
        ...
        await setDoc(doc(db, 'users', user.uid), {
          ...
          email: email,
          balance: 1000000, // Default starter balance
      ...
      <div className="w-full max-w-sm space-y-8">
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-white dark:bg-[#160d08] px-4 text-slate-400 font-bold">Or continue with</span>
          </div>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-5 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white font-bold rounded-2xl shadow-sm active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
        >
    ...
    <div className="flex flex-col gap-4 p-4 pb-24">
      <div className="flex items-center justify-between bg-slate-50 dark:bg-white/5 p-4 rounded-2xl border border-slate-200 dark:border-white/10">
        <div className="flex flex-col">
          ...
          <span className="text-lg font-bold">{spotPrice.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
        </div>
...
const PortfolioView = ({ portfolio, onClosePosition, userId }: { portfolio: Portfolio | null, onClosePosition: (id: string) => void, userId: string }) => {
...
const ChallengesView = ({ onSelectPlan, plans, rules }: { onSelectPlan: (name: string) => void, plans: Plan[], rules: Rule[] }) => {
  return (
    <div className="flex flex-col gap-6 p-4 pb-24">
              ...
              <div key={plan.id} className="p-5 rounded-3xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 shadow-sm">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <p className="text-xs text-primary font-black">₹{plan.price.toLocaleString()}</p>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-2 py-3 border-t border-slate-100 dark:border-white/5">
                  <div className="text-center">
                    ...
                    <p className="text-[10px] font-bold">₹{plan.capital.toLocaleString()}</p>
                  </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Real-time portfolio calculation from Firestore
    const tradesQuery = query(collection(db, 'trades'), where('userId', '==', user.uid), where('status', '==', 'Open'));
    const unsubscribeTrades = onSnapshot(tradesQuery, (snapshot) => {
      const openPositions = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Trade));
      
      // Calculate unrealized PnL based on current market data
      let unrealizedPnl = 0;
      openPositions.forEach(pos => {
        const currentPrice = marketData[pos.symbol]?.price || pos.price;
      ...
      if (userProfile) {
        setPortfolio({
          equity: userProfile.balance + unrealizedPnl,
  ...
  const handleClosePosition = async (tradeId: string) => {
    ...
    try {
      ...
      const currentPrice = marketData[trade.symbol]?.price || trade.price;
        ...
        {!isAuthReady ? (
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === 'challenges' && <ChallengesView onSelectPlan={(name) => showToast(`Challenge Started: ${name}`)} plans={plans} rules={rules} />}
              {activeTab === 'portfolio' && <PortfolioView portfolio={portfolio} onClosePosition={handleClosePosition} userId={user.uid} />}
